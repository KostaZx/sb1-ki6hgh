...
export default function Cart() {
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/store/basket/${basketId}/auth`, { params: { returnUrl: window.location.origin } });
    ...
    return (<div className="bg-white rounded-lg shadow-md p-6">
      ...
      {items.length === 0 ? (<p>Your cart is empty</p>) : (<>
          <div className="space-y-4">
            {items.map((item) => (<div key={item.id} className="flex justify-between items-center border-b pb-4">
                <div>
                  <h3 className="font-semibold">{item.name}</h3>
                  <p className="text-gray-600">${item.price}</p>
                </div>
                <div className="flex items-center space-x-4">
                  <input type="number" min="1" value={item.quantity} onChange={(e) => updateQuantity(item.id, parseInt(e.target.value))} className="w-16 px-2 py-1 border rounded"/>
          ...
          <div className="mt-6">
            ...
            <button onClick={handleCheckout} className="w-full mt-4 bg-blue-600 text-white py-2 px-4 rounded hover:bg-blue-700">
              Proceed to Checkout
            </button>
          </div>
        </>)}
